...
export function CsgGeometries({ color, doorBoundingBoxes, windowBoundingBoxes, ventBoundingBoxes, interior }) {
  ...
  const { nodes: cRightNodes } = useGLTF(
    "/models/container/40/exterior-right.glb"
  );
  const { nodes: cBackNodes } = useGLTF(
    "/models/container/40/exterior-back.glb"
  );
  const { nodes: cLeftNodes } = useGLTF(
    "/models/container/40/exterior-left.glb"
  );
  const { nodes: dRightNodes } = useGLTF("/models/drywall/40/drywall-right.glb");
  const { nodes: dLeftNodes } = useGLTF("/models/drywall/40/drywall-left.glb");
  const { nodes: dBackNodes } = useGLTF("/models/drywall/40/drywall-back.glb");

  const { nodes: pRightNodes } = useGLTF("/models/plywood/40/plywood-right.glb");
  const { nodes: pLeftNodes } = useGLTF("/models/plywood/40/plywood-left.glb");
  const { nodes: pBackNodes } = useGLTF("/models/plywood/40/plywood-back.glb");

  const csg = useRef();
  const { materials: containerMaterials } = useGLTF(
    "/models/container/40/container-whole.glb"
  );
        ...
        {interior === INTERIOR_OPTIONS[1] ? (
          <>
            <Base
              ...
              material={containerMaterials["White_Drywall_Wall.002"]}
              scale={10}
        ...
        ) : null}
        {
          interior === INTERIOR_OPTIONS[0] ? (
            ...
            <Base
              ...
              material={containerMaterials["Plywood_Texture_01.002"]}
              scale={10}
              position={[adjustForX, 0, adjustForY]}
            />
            <Base
