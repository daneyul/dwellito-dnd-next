import { DIMENSIONS, ELEVATION_NAMES } from "./library";
...
export const deScale = (value) => {
  return value / DIMENSIONS.SCALE_FACTOR;
}

export const checkDistance = ({ component, selectedElevation }) => {
  const droppableWidthValue = droppableWidth(selectedElevation); // Use the function to get dynamic width

  return {
    left: Math.round(deScale(component.position.x) + DIMENSIONS.BOUNDARIES.x).toFixed(),
    right: Math.round(droppableWidthValue - (deScale(component.position.x) + component.objWidth) + DIMENSIONS.BOUNDARIES.x).toFixed(),
    top: Math.round(deScale(component.position.y)).toFixed(),
  };
};
...
export const handleAddComponent = (
  ...
  setSelectedComponents((prevSelectedComponents) => {
    ...
    const collisionDetected = prevSelectedComponents.some((component) =>
      checkCollision(newItem, component, selectedElevation)
    );
...
export const DROPPABLE_SIDE_WIDTH_WITH_BOUNDARIES = DIMENSIONS.CONTAINER.SIDE.WIDTH - (DIMENSIONS.BOUNDARIES.x * 2);
export const DROPPABLE_BACK_WIDTH_WITH_BOUNDARIES = DIMENSIONS.CONTAINER.FRONT.WIDTH - (DIMENSIONS.BOUNDARIES.x * 2);

export const droppableWidth = (selectedElevation) => {
...
export const checkCloseness = (item1, item2, selectedElevation) => {
  ...
  const tooCloseHorizontally = Math.min(a.right, b.right) - Math.max(a.left, b.left) > -closenessThreshold;
  const tooCloseVertically = Math.min(a.bottom, b.bottom) - Math.max(a.top, b.top) > -closenessThreshold;

  return tooCloseHorizontally && tooCloseVertically;
};

export const snapToIncrement = (increment) => ({ transform }) => {
